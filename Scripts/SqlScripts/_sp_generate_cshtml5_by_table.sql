IF OBJECT_ID('[sm].[_sp_generate_cshtml5_by_table]', 'P') IS NOT NULL
	DROP PROCEDURE [sm].[_sp_generate_cshtml5_by_table]
GO
SET ANSI_NULLS ON
GO
...
sm._sp_generate_cshtml5_by_table 'd_inventory_type'


*/

CREATE PROCEDURE [sm].[_sp_generate_cshtml5_by_table]
...
as
	...
	declare
		...
		@namespaceNet		nvarchar(255) =  [sm].[_fn_net_namespace](@table_name)
...
namespace Tnomer.Remontica.Html5.Repository.' + @namespaceNet + '
{
    public class ' + @upper_case_name + 'Parameters : LikeNameParameters
    {'
		union all	select	'		public ' + sm._fn_to_dotnet_type(fulltype) + case when + sm._fn_to_dotnet_type(fulltype) = 'String' then '' else '?' end + ' ' + sm._fn_first_upper(sm._fn_to_camel_case(name)) +  ' { get; set; }' + char(13) + char(10) 
			from @t
			where left(name,2) <> 's_'
		union all	select	'
    	public ' + @upper_case_name + 'Parameters(){ }
    	public ' + @upper_case_name + 'Parameters(String include) :base(include){ }
    	public ' + @upper_case_name + 'Parameters(String include, String searchText) :base(include, searchText){ }

		public override List<UrlParameter> GetUrlParameters()
		{
			List<UrlParameter> col = base.GetUrlParameters();'
		union all	select	char(13) + char(10)  
						+	case when + sm._fn_to_dotnet_type(fulltype) = 'String' 
								then	
							'            if (!String.IsNullOrWhiteSpace(' + sm._fn_first_upper(sm._fn_to_camel_case(name)) + '))' + char(13) + char(10) 
								else
							'            if (' + sm._fn_first_upper(sm._fn_to_camel_case(name)) +  '.HasValue)' + char(13) + char(10) 
								end
						+	'                col.Add(new UrlParameter("filter_eq_' + sm._fn_to_camel_case(name) +  '", ' + sm._fn_first_upper(sm._fn_to_camel_case(name)) +  '.ToString()));' + char(13) + char(10) 
			from @t
			where left(name,2) <> 's_'
		union all	select '
            return col;
...
namespace Tnomer.Remontica.Html5.Models.' + @namespaceNet + '
{	
    public class ' + @upper_case_name + 'Model
    {
		public Int32 id { get; set; }'
		union all 	select	
'		public ' + Replace(sm._fn_to_dotnet_type(fulltype), 'Decimal', 'Double')  +  case when isnullable = 1 then '?' else '' end + ' ' + sm._fn_to_camel_case(name) + '{ get; set; }' + char(13) + char(10) 
					from 
						@t 
					where
						name not in( 'id') 
		...
		union all 	select	'' + char(13) + char(10) 
						+	'		//' + q.upper_case_name + char(13) + char(10) 
						+	'		public ' + q.upper_case_name + 'Model ' + q.camel_case_name + ' { get; set; }' + char(13) + char(10)
					from
						( 
							select -- c.name, refT.name, refC.name 
								refT.name,
		...
		union all 	select	'
		////' + q.upper_case_name + '
		//public ObservableCollection<' + q.upper_case_name + 'Model> ' + [sm].[_fn_many_suffix](q.camel_case_name )+ ' { get; set; }' 
					from @t_children q						
...
'	}
	...
	{	
		public ' + @upper_case_name + 'Model ' + @camel_case_name + ' { get; set; }
	}

	public class ' + @upper_case_name + 'ListResponseModel
	{	
		public ObservableCollection<' + @upper_case_name + 'Model> ' + [sm].[_fn_many_suffix](@camel_case_name )+ ' { get; set; }
	}
}
            ...
            //  ' + @upper_case_name + '
            //  ************************************************
            static Lazy2<GeneralRepository<' + @upper_case_name + 'Model, ResponseModel<' + @upper_case_name + 'ResponseModel>, ResponseModel<' + @upper_case_name + 'ListResponseModel>>>
                ' + @upper_case_name + 'Factory = new Lazy2<GeneralRepository<' + @upper_case_name + 'Model, ResponseModel<' + @upper_case_name + 'ResponseModel>, ResponseModel<' + @upper_case_name + 'ListResponseModel>>>
                (
                    () => new GeneralRepository<' + @upper_case_name + 'Model, ResponseModel<' + @upper_case_name + 'ResponseModel>, ResponseModel<' + @upper_case_name + 'ListResponseModel>>(ServiceResources.' + @namespaceNet + '_' + @upper_case_name + ')
                );
            public static GeneralRepository<' + @upper_case_name + 'Model, ResponseModel<' + @upper_case_name + 'ResponseModel>, ResponseModel<' + @upper_case_name + 'ListResponseModel>> ' + @upper_case_name + '
            {
                get
                {
                    return ' + @upper_case_name + 'Factory.Value;
                }
            }
------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------
<Page
    x:Class="Tnomer.Remontica.Html5.' + @namespaceNet + '.' + @upper_case_name + 'Page"
	...
	<tnomer:PopupContainer x:Name="PopupContainer1">
        <tnomer:PopupContainer.BasePanel>
			<Grid>
				...
				<tnomer:StripPanel x:Name="MainStripPanel" Margin="0,10,0,0" Grid.Row="0">
					<Grid HorizontalAlignment="Right">
							...
							<tnomer:Button x:Name="SaveButton" Grid.Column="0" 
											   Icon="&#xE74E;" Text="Сохранить"
											   Click="SaveButton_Click"
											   />
							<tnomer:Button x:Name="CancelButton" Grid.Column="1" 
											   Icon="&#xE711;" Text="Отмена"
				...
				<TabControl  x:Name="TabControl1" Margin="5,10,5,5" SelectedIndex="0"  Background="Transparent" Grid.Row="1">
					<TabItem  x:Name="TabItem1" Header="Главная" MinWidth="100" Background="Transparent">
		...
		union all 	select	
						'
                                <tnomer:XListBox x:Name="' + upper_case_name + 'List" Text="' + q.name + '" 
                                     DisplayMemberPath="Name" Items="{Binding ' + [sm].[_fn_many_suffix](q.camel_case_name ) + ', Mode=TwoWay}"
                                     RequestList="' + upper_case_name + 'List_RequestList">
                                    <tnomer:XListBox.DialogFilter>
                                        <WrapPanel >
                                            <tnomer:XTextBox x:Name="Filter' + upper_case_name + 'ListNameTextBox" Text="Имя" 
                                                 TextWrapping="NoWrap" AcceptsReturn="False" 
                                                 ValueChanged="Filter' + upper_case_name + 'ListNameTextBox_ValueChanged"
                                                 />
                                        </WrapPanel>
...
'			};
		...
		union all	select	
'		//  ******************************************************
		//  ' + q.upper_case_name + 'List
		//  ******************************************************
        LazyExecutor _' + q.camel_case_name + 'LookupRefreshLazyExecutor = new LazyExecutor();
        private async Task Refresh' + q.upper_case_name + 'Lookup()
        {
            try
            {
                var pars = new ' + q.upper_case_name + 'Parameters();

                pars.Name = Filter' + q.upper_case_name + 'ListNameTextBox.Value;

                var m = await (RemoteRepository.' + [sm].[_fn_net_namespace]( q.name) + '.' + q.upper_case_name + '.Get(pars));
                ' + q.upper_case_name + 'List.SetLookup(m.data.' + [sm].[_fn_many_suffix](q.camel_case_name ) + ');
            }
            catch (Exception ex)
            {
                await MessageBoxDialog.ShowAsync(ex.Message);
        ...
        private void ' + q.upper_case_name + 'List_RequestList(object sender, EventArgs e)
        {
            _' + q.camel_case_name + 'LookupRefreshLazyExecutor.SetHandler(Refresh' + q.upper_case_name + 'Lookup, 1);
        }

        private void Filter' + q.upper_case_name + 'ListNameTextBox_ValueChanged(object sender, TextChangedEventArgs e)
        {
            _' + q.camel_case_name + 'LookupRefreshLazyExecutor.SetHandler(Refresh' + q.upper_case_name + 'Lookup, 1);
        }'
					from
